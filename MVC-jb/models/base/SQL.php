...
class SQL implements IDatabase
{
    ...
    /**
     * Retourne l'ensemble des enregistrements présent en base de données n'ayant pas été terminée (pour la table $tableName)
     * Rajout pour évolution 1
     * @return array
     */
    public function getAllNonTermine()
    {
        $stmt = $this->pdo->prepare("SELECT * FROM {$this->tableName} WHERE termine!=1");
        $stmt->execute();
        return $stmt->fetchAll(\PDO::FETCH_ASSOC);
    }
